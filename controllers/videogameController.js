...
exports.videogame_update_get = asyncHandler(async (req, res, next) => {
    const [videogame, allPlatforms, allGenres, allEsrbs] = await Promise.all([
        Videogame.findById(req.params.id).populate("genre esrb").exec(),
        Platform.find().sort({ name: 1 }).exec(),
        Genre.find().sort({ name: 1 }).exec(),
        Esrb.find().sort({ name: 1 }).exec(),
    ]);
...
exports.videogame_update_post = [
    ...
    asyncHandler(async (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            ...
            const updatedGame = await Videogame.findByIdAndUpdate(req.params.id, videogame, {});
